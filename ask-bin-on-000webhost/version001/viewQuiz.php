    ...
    <?php
        if (isset($_GET['name'])) {
            ...
            while($row = mysqli_fetch_row($result)){
                ...
                echo '<button class="btn cta" onclick="javascript:window.print();">Print</button>';
                echo '</section><hr>';
            }
            // Free result set
            unset($result);
            ...
            echo '</section>';
            echo '<input type="text" style="display:none;" id="urltxt" value="'.(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]&owner=".$owner.'&shared=yes"/>';
        } else {
